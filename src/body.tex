...
distributed and alternative energy sources, intelligent coordination systems, cybersecurity  and electric vehicles\cite[p.10ff.]{mitei2011}. New problem solving techniques are required to solve such \emph{wicked problems}, because
...
now called Deep Reinforcement Learning. This field has seen success in recent research, allowing for agents that
...
same kind but not identical. Common examples of this form of learning include object recognition in images or
...
labels. If a picture of a dog is read by the \ac {NN}, it needs to be able to classify the fact that a dog is in the
picture. In areas such as financial trading or electricity demand prediction, it can be helpful to be able to predict
future patterns based on current and previous observations. In the space of machinery, learning to recognize sensor data
that indicates faulty parts can be used to avoid down-time of machines through preemptive replacement during scheduled
service intervals. In the online marketing industry, recognizing user interests to send appropriate ads benefits just as
well from the approach as do spam filters that recognize ads and filter them out again.
...
If $y$ can be any of a set of answers, the problem is a \emph{classification} problem and if the problem requires the
...
large and the general principle, called Ockhams razor, is that simpler hypotheses with equal performance as more complex
ones are to be preferred. By deciding up-front about the decision space (e.g.\ all linear functions) the best hypothesis
might not be able to perfectly match the underlying true function $f$. On the other hand a hypothesis chosen from a
expressive hypotheses space may generalize very well and is easier to understand and implement.
...
overall goal of unsupervised learning is therefore not to predict but to learn information about the underlying distributions and
...
and the other way around. Further, the \emph{max} operator creates non linearity which, for large state spaces, becomes
...
years, many breakthroughs have been driven by using neural networks in \ac{RL} settings. Neural networks have proven effective as
...
\begin{figure}[h]%!h \centering
    ...
    \caption{PowerTAC overview of markets}
...
brokers. This is based on the assumption that the grid infrastructure has a static capacity that is required to support
the scale it does due to the peak transmission demands. Brokers are therefore incentivized to create portfolios that
don't exhibit large deltas between different hours of the day or days of the week.
...
further analysis. I created records using the powertac-tools project for all games downloadable for 2017 to let the
...
historical data based environment approximation, the market prices don't get influenced by the brokers placement of ask
...
the broker offered tariff X in time slot 1200, it would have won the competition"}, it is not possible to reproduce the
...
system to be recreated and to be in the exact same state as the original. This means, a \ac{RL} agent can learn not simply
...
\subsection{Tensorforce and kerasl-rl}%
\label{sub:tensorforce_and_kerasl_rl}

Tensorforce and kerasl-rl are both relatively young libraries that intend to offer a high-level \ac{API} for building
...
replaced by a new framework "YARL", but the \ac{API} will be similar\footnote{Information received during private
...
\ac{PowerTAC} entities, reducing the scope while retaining the benefits of a \ac{GRPC} implementation. Since \ac{GRPC}
is \ac{JSON} compatible, it appears to be the best choice.
...
{PowerTAC} entities don't follow the bean specification for getters and setters or where getters and setters are simply
...
env module of the broker as described before. Messages can be considered as action triggers and are therefore
...
loosing the ability to easily share it with other developers. One broker already depends on R, a common statistics tool
...
a surrounding architecture had to be built to allow this network to both learn and predict live in a game setting.
...
specific time slot rather than information about earlier and later slots as well.
...
time slot. The \texttt{WholesaleEnvironmentManager} therefore abstracts the multiplicity complexity from the individual
\texttt{PowerTacEnv}.
...
server and therefore the \ac{RL} agent itself has no control of the flow. The forward and backward methods are
...
therefore leads to a changed policy after the first update has been completed, it could theoretically be batched into
...
real-world energy market which means the score equals the brokers profit. But the profit is dependent on a number of
...
overall market therefore describes how well the agent did in comparison to the others and therefore removes the market
...
buying sufficient energy for the target time slot to ensure its portfolio is balanced. The reward function may therefore
...
\begin{table}[]
    ...
    \resizebox{\textwidth}{0.48\textheight}{
        \begin{tabular}{l|l|l|l|l|l}
            \textbf{Network} & \textbf{r\_type} & \textbf{Preprocessing} & \textbf{Action\_T} & \textbf{agent\_type} & \textbf{r after 30 games} \\ \hline
            vnn32x2          & $r_{comb}$      & simple                 & continuous         & dqn                 & 0.0 \\
            vnn32x2          & $r_{comb}$      & simple                 & continuous         & vpg                 & 0.0 \\
            vnn32x2          & $r_{comb}$      & simple                 & continuous         & random              & 0.0 \\
            vnn32x2          & $r_{comb}$      & simple                 & discrete           & dqn                 & 0.0 \\
            vnn32x2          & $r_{comb}$      & simple                 & discrete           & vpg                 & 0.0 \\
            vnn32x2          & $r_{comb}$      & simple                 & discrete           & random              & 0.0 \\
            vnn32x2          & $r_{comb}$      & simple                 & twoarmedbandit     & dqn                 & 0.0 \\
            vnn32x2          & $r_{comb}$      & simple                 & twoarmedbandit     & vpg                 & 0.0 \\
            vnn32x2          & $r_{comb}$      & simple                 & twoarmedbandit     & random              & 0.0 \\
            vnn32x2          & $r_{comb}$      & simplenorm             & continuous         & dqn                 & 0.0 \\
            vnn32x2          & $r_{comb}$      & simplenorm             & continuous         & vpg                 & 0.0 \\
            vnn32x2          & $r_{comb}$      & simplenorm             & continuous         & random              & 0.0 \\
            vnn32x2          & $r_{comb}$      & simplenorm             & discrete           & dqn                 & 0.0 \\
            vnn32x2          & $r_{comb}$      & simplenorm             & discrete           & vpg                 & 0.0 \\
            vnn32x2          & $r_{comb}$      & simplenorm             & discrete           & random              & 0.0 \\
            vnn32x2          & $r_{comb}$      & simplenorm             & twoarmedbandit     & dqn                 & 0.0 \\
            vnn32x2          & $r_{comb}$      & simplenorm             & twoarmedbandit     & vpg                 & 0.0 \\
            vnn32x2          & $r_{comb}$      & simplenorm             & twoarmedbandit     & random              & 0.0 \\
            vnn32x2          & $r_{rel}$       & simple                 & continuous         & dqn                 & 0.0 \\
            vnn32x2          & $r_{rel}$       & simple                 & continuous         & vpg                 & 0.0 \\
            vnn32x2          & $r_{rel}$       & simple                 & continuous         & random              & 0.0 \\
            vnn32x2          & $r_{rel}$       & simple                 & discrete           & dqn                 & 0.0 \\
            vnn32x2          & $r_{rel}$       & simple                 & discrete           & vpg                 & 0.0 \\
            vnn32x2          & $r_{rel}$       & simple                 & discrete           & random              & 0.0 \\
            vnn32x2          & $r_{rel}$       & simple                 & twoarmedbandit     & dqn                 & 0.0 \\
            vnn32x2          & $r_{rel}$       & simple                 & twoarmedbandit     & vpg                 & 0.0 \\
            vnn32x2          & $r_{rel}$       & simple                 & twoarmedbandit     & random              & 0.0 \\
            vnn32x2          & $r_{rel}$       & simplenorm             & continuous         & dqn                 & 0.0 \\
            vnn32x2          & $r_{rel}$       & simplenorm             & continuous         & vpg                 & 0.0 \\
            vnn32x2          & $r_{rel}$       & simplenorm             & continuous         & random              & 0.0 \\
            vnn32x2          & $r_{rel}$       & simplenorm             & discrete           & dqn                 & 0.0 \\
            vnn32x2          & $r_{rel}$       & simplenorm             & discrete           & vpg                 & 0.0 \\
            vnn32x2          & $r_{rel}$       & simplenorm             & discrete           & random              & 0.0 \\
            vnn32x2          & $r_{rel}$       & simplenorm             & twoarmedbandit     & dqn                 & 0.0 \\
            vnn32x2          & $r_{rel}$       & simplenorm             & twoarmedbandit     & vpg                 & 0.0 \\
            vnn32x2          & $r_{rel}$       & simplenorm             & twoarmedbandit     & random              & 0.0 \\
            bn\_vnn32x2      & $r_{comb}$      & simple                 & continuous         & dqn                 & 0.0 \\
            bn\_vnn32x2      & $r_{comb}$      & simple                 & continuous         & vpg                 & 0.0 \\
            bn\_vnn32x2      & $r_{comb}$      & simple                 & continuous         & random              & 0.0 \\
            bn\_vnn32x2      & $r_{comb}$      & simple                 & discrete           & dqn                 & 0.0 \\
            bn\_vnn32x2      & $r_{comb}$      & simple                 & discrete           & vpg                 & 0.0 \\
            bn\_vnn32x2      & $r_{comb}$      & simple                 & discrete           & random              & 0.0 \\
            bn\_vnn32x2      & $r_{comb}$      & simple                 & twoarmedbandit     & dqn                 & 0.0 \\
            bn\_vnn32x2      & $r_{comb}$      & simple                 & twoarmedbandit     & vpg                 & 0.0 \\
            bn\_vnn32x2      & $r_{comb}$      & simple                 & twoarmedbandit     & random              & 0.0 \\
            bn\_vnn32x2      & $r_{comb}$      & simplenorm             & continuous         & dqn                 & 0.0 \\
            bn\_vnn32x2      & $r_{comb}$      & simplenorm             & continuous         & vpg                 & 0.0 \\
            bn\_vnn32x2      & $r_{comb}$      & simplenorm             & continuous         & random              & 0.0 \\
            bn\_vnn32x2      & $r_{comb}$      & simplenorm             & discrete           & dqn                 & 0.0 \\
            bn\_vnn32x2      & $r_{comb}$      & simplenorm             & discrete           & vpg                 & 0.0 \\
            bn\_vnn32x2      & $r_{comb}$      & simplenorm             & discrete           & random              & 0.0 \\
            bn\_vnn32x2      & $r_{comb}$      & simplenorm             & twoarmedbandit     & dqn                 & 0.0 \\
            bn\_vnn32x2      & $r_{comb}$      & simplenorm             & twoarmedbandit     & vpg                 & 0.0 \\
            bn\_vnn32x2      & $r_{comb}$      & simplenorm             & twoarmedbandit     & random              & 0.0 \\
            bn\_vnn32x2      & $r_{rel}$       & simple                 & continuous         & dqn                 & 0.0 \\
            bn\_vnn32x2      & $r_{rel}$       & simple                 & continuous         & vpg                 & 0.0 \\
            bn\_vnn32x2      & $r_{rel}$       & simple                 & continuous         & random              & 0.0 \\
            bn\_vnn32x2      & $r_{rel}$       & simple                 & discrete           & dqn                 & 0.0 \\
            bn\_vnn32x2      & $r_{rel}$       & simple                 & discrete           & vpg                 & 0.0 \\
            bn\_vnn32x2      & $r_{rel}$       & simple                 & discrete           & random              & 0.0 \\
            bn\_vnn32x2      & $r_{rel}$       & simple                 & twoarmedbandit     & dqn                 & 0.0 \\
            bn\_vnn32x2      & $r_{rel}$       & simple                 & twoarmedbandit     & vpg                 & 0.0 \\
            bn\_vnn32x2      & $r_{rel}$       & simple                 & twoarmedbandit     & random              & 0.0 \\
            bn\_vnn32x2      & $r_{rel}$       & simplenorm             & continuous         & dqn                 & 0.0 \\
            bn\_vnn32x2      & $r_{rel}$       & simplenorm             & continuous         & vpg                 & 0.0 \\
            bn\_vnn32x2      & $r_{rel}$       & simplenorm             & continuous         & random              & 0.0 \\
            bn\_vnn32x2      & $r_{rel}$       & simplenorm             & discrete           & dqn                 & 0.0 \\
            bn\_vnn32x2      & $r_{rel}$       & simplenorm             & discrete           & vpg                 & 0.0 \\
            bn\_vnn32x2      & $r_{rel}$       & simplenorm             & discrete           & random              & 0.0 \\
            bn\_vnn32x2      & $r_{rel}$       & simplenorm             & twoarmedbandit     & dqn                 & 0.0 \\
            bn\_vnn32x2      & $r_{rel}$       & simplenorm             & twoarmedbandit     & vpg                 & 0.0 \\
            bn\_vnn32x2      & $r_{rel}$       & simplenorm             & twoarmedbandit     & random              & 0.0 \\
...
%TODO analyze which parts perform better, which worse
%TODO brainstorm reasons for the failure
...
In the beginning of this work, I have described the research progress in \ac{AI} and how new neural network based systems are able to solve
problems, previously only solvable by humans. I have also indicated the obvious incentive to apply these new
techniques to important contemporary problems, focusing on future energy markets as one such problem. \ac{PowerTAC}, a
simulation of these markets was introduced as a core research initiative that attempts to explore this problem field by
letting many researchers compete in a competitive setting and simulate profit-oriented market participants. Because
future market participants will not shy away from using \ac{AI} technologies to improve their competitiveness,
\ac{PowerTAC} contenders must also be enabled to pursue these technologies to realistically represent future markets. To
enable new competition participants to quickly catch up and to generally enable the use of neural networks, I
decided to pursue the question whether imitation based \ac{RL} may be deployable in the \ac{PowerTAC} setting.
